<?php
	...
	if(isset($_POST['load_l_log'])){
    ...
    }else if(isset($_POST['load_l_log_files'])){
        $select="l_log_files.lf_file";
		$option=" where
		l_log_files.l_id='{$_POST['l_id']}'";
		echo $db->select("l_log_files",$select,$option);
		...
		if($res->status){
            ...
            if(isset($_FILES) && $_FILES['lf_file']['name']!=''){
				// $db->get_arr($_FILES);
				foreach ($_FILES['lf_file']['tmp_name'] as $key => $val) {
					$filename = "../../img/".date("Y-m-d_His")."-".$key.".jpg";
					if(move_uploaded_file($val, $filename)){
						$lf_files=$filename;
